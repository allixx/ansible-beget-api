...
def private_dns_to_beget(records, subdomain):
    ...
    if not isinstance(records, list):
        raise AnsibleFilterTypeError(
            f"dns_to_beget requires a list, got {type(records)} instead."
        )

    for rec in [
        r
        for r in records
        if r.keys() & ["subdomain", "type"] and r["subdomain"] == subdomain
    ]:
        rec_type = rec["type"]
        rec_value = flatten([rec["value"]])
